...
//生成数字签名
func SignatureRSA() (string, error) {
	// 1. 从秘钥文件中读生成的秘钥内容
	fp, err := os.Open("file/encryption/private.pem")
	if err != nil {
		return "", errors.New("打开私钥文件 - private.pem 失败!!!")
	}
	// 2. 读文件内容
	fileInfo, _ := fp.Stat()
	...
	return string(mySignature), nil
}

//验证数字签名
func VerifyRSA(src string) (error){
	// 1. 从秘钥文件中读生成的秘钥内容
	...
	// 7. 数据认证
	err = rsa.VerifyPKCS1v15(pubKey, crypto.SHA256, result, []byte(src))
	if err != nil {
